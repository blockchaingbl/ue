<template>
  <div class="g-inherit m-article g-window">
    <x-header  class="m-tab" :left-options="leftBtnOptions" @on-click-back = "onClickBack">
      ...
      <a slot="left"></a>
      <div  slot="right">
        <span v-if="scene==='team'"  @click.prevent="onTeamManageClick">群设置</span>
      </div>
    </x-header>
...
export default {
  ...
  methods: {
    onClickBack () {
      ...
      this.$http.post('api/app.user/nim/cp',{}).then(res=>{
        if(res.errcode==0)
        {
    ...
    enterNameCard () {
      if (/^p2p-/.test(this.sessionId)) {
          console.log(account)
        let account = this.sessionId.replace(/^chat-fullscreen/, '')
          console.log(account)

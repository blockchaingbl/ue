<template>
  <div class='g-inherit m-article p-membercard g-window'>
    ...
    <div class='g-body'>
      ...
      <group class='m-group'>
        <cell title="群昵称" :value="member.nickInTeam||'未设置'" @click.native="()=> hasSetNickPermission? onEditItemClick('修改群昵称', 'text', 'nickInTeam', getUpdateCallBcak()) : $vux.toast.text('无权限')" is-link></cell>
        <cell title="身份" :value="memberType"  @click.native="()=> hasSetMemberTypePermission? onEditItemClick('身份', 'select', 'memberType', getUpdateCallBcak()) : $vux.toast.text('无权限')" is-link></cell>
        ...
        options: {
          ...
          mute: this.mute,
            ...
            if(error) {
              ...
              this.$vux.toast.text(this.mute? '已禁言':'已取消禁言')
            }
          }
        }
      })
      ...
      this.$store.dispatch('delegateTeamFunction', {
        ...
        options: {
          ...
          accounts: [this.member.account],
            ...
            this.$vux.toast.text(error ? error : '移除成功')
            history.go(-1)

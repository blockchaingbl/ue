<template>
  <div class='g-inherit m-article p-teammanager g-window'>
          ...
          fileInput,
            ...
            if(err) {
              ...
              if(data.w<300||data.h<300){
                  this.$vux.toast.text("图片长宽不能小于300")
                  return
              }
      ...
      this.$store.dispatch('delegateTeamFunction', {
        ...
        options: {
          ...
          avatar: url,
            ...
            this.$vux.toast.text(err? err: '修改群头像成功')
          }
        }
      })
    },
          ...
          that.$store.dispatch('delegateTeamFunction', {
            ...
            options: {
              teamId: that.teamId,
                ...
                that.$vux.toast.text(error? error: '已解散群')
                window.history.go(-1)
              }
            }
          })
          ...
          that.$store.dispatch('delegateTeamFunction', {
            ...
            options: {
              teamId: that.teamId,
                ...
                that.$store.dispatch('hideLoading')
                that.$vux.toast.text(error? error: '已退出群')
                window.history.go(-2)
              }
            }
          })
...
<style lang="less" scoped>
  ...
  .g-window .m-article .p-teammanager .g-inherit {
    display:flex;

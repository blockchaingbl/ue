<template>
  <div class="g-inherit m-article p-namecard g-window">
    <x-header class="m-tab" :left-options="{backText: ' '}">
      <h1 class="m-tab-top" style="color: #fff">{{userInfo.alias}}</h1>
      <a slot="left"></a>
    </x-header>
    ...
    <div v-else class="m-list">
      <group class="u-card">
        <cell :title="'昵称: '+userInfo.nick" :value="userInfo.gender=='不显示'?'':userInfo.gender">
      ...
      <div class="u-bottom" style="text-align: center">
...
export default {
  ...
  methods: {
    ...
    enterChat () {
        ...
        this.$router.push({path: '/chat/p2p-'+this.account})
    },
    enterHistory () {
        this.$router.push({path: `/chatHistory/p2p-${this.account}`})
    },
    addFriend () {

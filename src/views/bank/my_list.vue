<template lang="html">
<div class="page-bank-self" >
    <div class="content">
        <div class="detail-con" v-if="!loadings">
            <tab :line-width="2" :animate="false">
                <tab-item @click.native="tab()"><span>资产宝</span></tab-item>
                <tab-item selected><span>我的聚宝</span></tab-item>
            </tab>
            <div class="con-box" v-if="current_list.length>0||fixed_list.length>0">
                <div class="profit-block" v-if="current_list.length>0">
                    ...
                    <router-link :to="{path: '/finance/current_detail/'+item.btc_bank.id}" class="profit-item flex-box" v-for="(item,index) in current_list" :key="index">
                        <div class="profit-item-item">
                            <div class="item-item-title income-rate">{{item.btc_bank.year_rate}}%</div>
                            <div class="item-item-decs">年化创收率</div>
                        </div>
                        <div class="profit-item-item flex-1">
                            <div class="item-item-title">{{item.btc_bank.name}}</div>
                            <div class="item-item-decs" v-if="item.btc_bank.is_per_limit==0">随存随取 | 不限额</div>
                            <div class="item-item-decs" v-if="item.btc_bank.is_per_limit==1">随存随取 | 限购{{item.btc_bank.per_limit}}{{item.coin_unit}}</div>
                        </div>
                        <div class="profit-item-item t_r">
                            <div class="item-item-decs">
                                <div  class="item-item-btn">查看</div>
                            </div>
                        </div>
                    </router-link>
                </div>
                <div class="profit-block"  v-if="fixed_list.length>0">
                    ...
                    <router-link  :to="{path: '/finance/detail/'+item.btc_bank.id}" class="profit-item flex-box" v-for="(item,index) in fixed_list" :key="index">
                        <div class="profit-item-item">
                            <div class="item-item-title income-rate">{{item.rate}}%</div>
                            <div class="item-item-decs">年化创收率</div>
                        </div>
                        <div class="profit-item-item flex-1">
                            <div class="item-item-title">{{item.name}}</div>
                            <div class="item-item-decs" v-if="item.btc_bank.is_per_limit==0">{{item.btc_bank.time_length}} 天 | 不限额</div>
                            <div class="item-item-decs" v-if="item.btc_bank.is_per_limit==1">{{item.btc_bank.time_length}} 天 | 限购{{item.btc_bank.per_limit}}{{item.coin_unit}}</div>
                        </div>
                        <div class="profit-item-item t_r">
                            <div class="item-item-decs">
                                <div  class="item-item-btn">查看</div>
            ...
            <nodata  v-else :datatip="'暂无投资'"></nodata>
        </div>
        <div v-transfer-dom>
    ...
    <popup class="pop-deposit-deploy" v-model="showDeploy" position="bottom" style="width:100%;background:#fff;"  v-transfer-dom>
        ...
        <group>
            <cell title="可用数额" value="100,000.000"></cell>
            <div class="deposit-deploy-tis">限购 10000 {{$store.state.init.coin_uint}}，剩余 100 {{$store.state.init.coin_uint}}</div>
        ...
        <group class="nobg flex-box">
            <x-button type="primary" style="border-radius:99px;height:2.25rem;line-height:2.25rem;font-size:0.875rem;background:#3f73ed">确认</x-button>
        </group>
    </popup>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        getOrder(){
            this.$http.post('/api/app.financial/btcbank/order',{}).then(res=>{
                console.log(res)

<template lang="html">
    ...
    <div :id="page_name">
        ...
        <div class="pay_rece_address">
            <div class="pay_rece_item">
              <div class="pay_rece_title">付款地址</div>
              <div class="pay_rece_input">
                <x-input type="text" readonly="readonly" :value="shortStr(this.$store.state.wallet.address,12,20,'...')"></x-input>
              </div>
            </div>
            <div class="pay_rece_item">
              <div class="pay_rece_title">转入地址</div>
              <div class="pay_rece_input">
                <x-input placeholder="输入以太坊地址或 ENS 域名" v-model="recieve_address" :required="true" ref="recieve_address"></x-input>
              </div>
            </div>
        </div>
        <div class="send-assets-box">
          <div class="assets-num flex-box">
            <div class="assets-num-title flex-1">{{unit}}</div>
            <div class="assets-money">余额 {{coin_amount}}</div>
          </div>
          <group>
            <x-input placeholder="转出数额" v-model="recieve_amount" type="number" class="recieve_input recieve-price"  :required="true" ref="recieve_amount" style="font-size:1.25rem;"></x-input>
            <x-input placeholder="钱包密码"  type="password" class="recieve_input" v-model="password"></x-input>
          </group>
        </div>
        <div class="recieve_input_block">
            <div class="cost-input"  v-show="displayType==0" :class='{active:displayType==0}' >
    ...
    if (!this.$store.state.wallet.address) {
      this.$store.commit("loadWallets", function() {
        ...
        _this.$store.commit("loadProperties", function() {
          ...
          _this.unit=_this.$store.state.properties_set[coin_type].name;
          _this.coin_amount =_this.$store.state.properties_set[coin_type].amount;
      ...
      window.js_qr_code_scan = function(qr_code) {
        if (qr_code.substr(0, 2) == "0x") {
          ...
          qr_code = qr_code.replace(
            _this.$store.state.init.route_domain + "/#",
            ""
          );

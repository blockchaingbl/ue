            ...
            <div class="distri-block">
                <a href="#/candy/distribute_detail"  @click.prevent="turnDetail(sugar)" class="item" v-for="sugar,index in sugars" :key="index">
                    <div class="item-item flex-box">
                        ...
                        <div class="item-text" >转入人：{{sugar.to_user.username}}</div>
                    </div>
                    <div class="item-item flex-box">
                        <div class="item-text flex-1">资产({{sugar.coin_info.coin_unit}})：{{sugar.amount}}</div>
                        <div class="item-text">手机：{{sugar.to_user.mobile}}</div>
                    </div>
                    <div class="item-item flex-box">
                        <div class="item-text flex-1">锁定：{{sugar.lock_time}} {{word_obj[sugar.sugar_type]}}</div>
    ...
    methods: {
        loadList:function () {
            if (this.loading) {
                ...
                //加载完毕
                this.loading=true;
                this.$http.post('/api/app.user/locktransfer',this.formData).then(res => {

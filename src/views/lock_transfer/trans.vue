            ...
            <div class="cand-item flex-box" v-for="(item,index) in sugars_list">
                ...
                <div class="item-text flex-1 flex-box vux-1px-b">
                    <div class="item-info flex-1">
                        <div class="candy-title">{{item.coin_info.coin_unit}}: {{item.amount}}</div>
                        <div class="candy-decs"><div>锁仓{{item.lock_time}}{{word_obj[item.sugar_type]}}</div>
                            <div>来自： {{item.from_user.username}}</div>
                        </div>
                    </div>
                    <div class="item-btn-box">
                        <div class="item-btn-decs">{{item.desc}}</div>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        getSugars(){
            ...
            this.$http.post('/api/app.user/locktransfer',this.formData)
                .then(res=>{

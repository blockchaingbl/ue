<template>
  <div class="normal-packet">
    ...
    <p class="normal-remark">每人数额相等</p>
    <div class="normal-group">
      <router-link to="" class="normal-link">
        <p>总数额</p>
        <span><input v-model="number1" type="number" placeholder="0"/>&nbsp;{{inputValue[0]}}</span>
      </router-link>
    </div>
    <p class="normal-remark">托管数额：<span class="normal-number">{{ amount }}{{inputValue[0]}}</span><span class="red-pay">兑换</span></p>
    <div class="normal-group">
      <router-link to="" class="normal-link">
        <p>红包个数</p>
    ...
    <textarea class="normal-message" placeholder="恭喜发财，吉祥如意"></textarea>
  ...
  <div v-transfer-dom>
    <div v-show="isShow">
        <normal-dialog v-on:close="closeDialog" :number1="number1" :number2="number2" :inputValue="inputValue"></normal-dialog>
    </div>
 </div> 
...
import normalDialog from '@/views/red_packet/inc/normalDialog'
export default {
  directives: {
        TransferDom
    },

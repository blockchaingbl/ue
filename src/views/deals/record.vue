...
export default {
    ...
    methods:{
        loadOtcOrders(){
            ...
            this.lock = true;
            this.loading = true;
            this.$http.post('/api/app.otc/order',this.formData)
                .then(res=>{
        ...
        appeal(id){
            ...
            this.$vux.confirm.show({
                ...
                onConfirm () {
                    ...
                    _this.$http.post('/api/app.otc/order/appeal',formData)
                        .then(res=>{
        ...
        turnOrder(order){
            if(this.formData.type==1 && (order.status==0 || order.status==1)){
                this.$router.push({name:'dealsBuy',params:{otc_order_id:order.id,type:1}})
            }else if(this.formData.type==2 && order.status==1){
                this.$router.push({name:'dealsBuy',params:{otc_order_id:order.id,type:2}})
            }
        },
        cancel(order){
            const _this = this
            this.$vux.confirm.show({
                ...
                onConfirm () {
                    ...
                    _this.$http.post('/api/app.otc/order/cancel',formData)
                        .then(res=>{
                            if(res.errcode==0){
                                _this.$vux.toast.text('已取消');
                                setTimeout(function () {
                                    _this.reload(_this.formData.type);
                                },2000)
                            }else{

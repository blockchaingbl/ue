<template lang="html">
    <div :id="page_name">
        ...
        <group>
            <cell is-link  :link="{path:'/wallet/manage/'+address}" v-for="(wallet, address) in this.$store.state.wallets" :key="address">
                <span slot="title">
                    <div class="wallet_name">{{wallet.name}}</div>
                    <div class="wallet_address">{{address}}</div>
                </span>
            </cell>
        </group>
...
<script>
    import { XHeader,Actionsheet,Cell,Badge} from 'vux';

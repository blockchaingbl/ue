...
<div class="buy-box">
    ...
    <div v-show="buy_flag==0 && !time_end">
        <div class="buy-bottom">
            <div class="tis">转入不发{{coin_unit}}将视为严重影响信用值,请在300分钟内完成发{{coin_unit}}</div>
            <div class="count-down">
                倒计时 <span>{{minute}}:{{second}}</span>
            </div>
            <box gap="8px 35px 25px">
                <x-button type="primary" style="border-radius:99px;" class='buy-btn' :class="{ 'noTime': add }" @click.native="deliver()">我已收到款，确认发{{coin_unit}}</x-button>
            </box>
        </div>
    </div>
    <div class="pupBox" :class="{ 'isopen': is_open }">
        <div class="pupBox-content">
            <div class="pup-block">
                <p>您受让的{{coin_unit}}将在{{this.$store.state.init.otc_order_overtime}}分钟内到账，请注意查收。</p>
...
<script>
    ...
    mounted () {
      ...
      this.$http.post('/api/app.tokenotc/order/info',{order_id:this.order_id})
        .then(res=>{
          ...
          {
            type=2;
            this.sellerName = otc_order.buyer.username;
            this.accid =otc_order.buyer.accid
          }else{
            type=1;
            this.sellerName = otc_order.seller.username;
            this.accid =otc_order.seller.accid
          }
          this.coin_unit = otc_order.coin_unit
          ...
          if(type==1){
            if(otc_order.status>0){
              this.pay_status = otc_order.status;
              this.confirm_pay = 1;
              this.buttonName = '已付款';
            }else{
            ...
            let diff_time = end_time - timestamp;
            if(diff_time<0){
              this.$vux.toast.text(`已超出发${this.coin_unit}时间，如已收款，请在意见反馈里申请客服协助`);
              this.time_end = true;
            }else{
              this.counttime(diff_time);
            }
    ...
                }, 1000)
        ...
        deliver(){
            ...
            const _this = this
            this.$vux.confirm.show({
                content: `是否确认发`+this.coin_unit,
                onConfirm () {
                    _this.$http.post('/api/app.tokenotc/deals/confirmreceive',formData)
                        .then(res=>{
                            if(res.errcode==0){
                                _this.$vux.toast.text(`已确认发`+_this.coin_unit);
                                setTimeout(function () {
                                    _this.$router.push({path:'/token_otc/order'});
                                },2000)
                            }else{

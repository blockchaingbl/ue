...
<div class="page-candy-distribute">
    <x-header :left-options="{showBack: true,backText:''}"  style="z-index:1;">
        <span v-if="$route.params.type==1">加油卡</span>
        <span v-else>商超购物卡</span>
    </x-header>
            ...
            <div class="distri-block">
                <a href="#" class="item" v-for="order,index in orders" :key="index">
                    <div class="item-item flex-box">
                        <div class="item-text flex-1">市县:{{order.area}}</div>
                        <div class="item-text flex-1">姓名：{{order.name}}</div>
                    ...
                    <div class="item-item flex-box">
                        ...
                        <div class="item-text">
                            手机号码：<a style="color: #363840;" :href="'tel:'+order.mobile">{{order.mobile}}</a>
                        </div>
                    </div>
                    <div class="item-item flex-box">
    ...
    methods: {
        loadList:function () {
            if (this.loading) {
                ...
                //加载完毕
                this.loading=true;
                this.$http.post('/api/app.apply/credit/card',this.formData).then(res => {

    ...
    <div class="setalipay-box">
        ...
        <div class="block">
            <group class="alipay-item">
                <x-input title='微信账户' text-align="right" v-model="payment_account" placeholder="请输入您的微信账户" :max="20"></x-input>
            </group>
        </div>
        <div class="block">
            <group class="alipay-item no-line">
...
export default {
  ...
  methods:{
      getPaymentInfo(){
          this.$http.post('/api/app.payment/payment/bindinfo',{"payment_key":"weixin"}).then(res => {
              if(res.data.bind_info.weixin){
                  this.payment_receipt=res.data.bind_info.weixin.payment_receipt;
                  this.payment_account=res.data.bind_info.weixin.payment_account;
                  this.qrcode=res.data.bind_info.weixin.qrcode;
                  this.is_connect = Boolean(res.data.bind_info.weixin.is_connect);
                  this.connect = res.data.bind_info.weixin.connect;
              }
          }).catch(err => {
                if (err.errcode) {

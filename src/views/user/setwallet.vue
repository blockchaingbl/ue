...
export default {
  ...
  methods:{
      getPaymentInfo(){
          this.$http.post('/api/app.payment/payment/bindinfo',{"payment_key":"wallet"}).then(res => {
              if(res.data.bind_info.wallet){
                  this.qrcode=res.data.bind_info.wallet.qrcode;
                  this.is_connect = Boolean(res.data.bind_info.wallet.is_connect);
                  this.connect = res.data.bind_info.wallet.connect;
              }
          }).catch(err => {
                if (err.errcode) {

...
import {setScrollloding, setToken,setInit,setSign,setCoinInfo,refreshCoinType} from './actions'
function connectNim ({state, commit, dispatch}, obj) {
  ...
  if (!state.nim || force) {
    ...
    if (!loginInfo.uid && !token) {
      // 无cookie，直接跳转登录页
      pageUtil.turnPage('', 'regist')
    } else if(!loginInfo.uid){

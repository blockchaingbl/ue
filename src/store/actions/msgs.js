...
function onSendMsgDone (error, msg) {
  ...
  if (error) {
    ...
    if (error.code === 7101) {
      ...
      Vue.$vux.toast.text(error.message)
      // Vue.$vux.toast.text('网络异常,请尝试重连')
      Vue.$router.push({path: '/error/404'})
    }
  }
  onMsg(msg)
}
...
export function onRevocateMsg (error, msg) {
  ...
  if (error) {
    if (error.code === 508) {
      Vue.$vux.toast.text('发送时间超过2分钟的消息，不能被撤回')
    } else {
    }
    return
  }

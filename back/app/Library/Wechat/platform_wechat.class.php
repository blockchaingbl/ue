...
use \App\Http\Models\Common\WeixinAccount;
...
class PlatformWechat
{
    ...
    const MSGTYPE_transfer_customer = 'transfer_customer_service';
    ...
    const USER_GET_URL = '/user/get?';       //获取公众号粉丝列表
    const USER_INFO_URL = '/user/info?';     //获取粉丝的基本信息
    ...
    const TEMPLATE_GET_ALL_URL = '/template/get_all_private_template?';
...
//		);
        ...
        //@file_put_contents(base_path("/storage/logs/a.txt"),$errCode,FILE_APPEND);
        if ($errCode == 0) {
    ...
    {
        if (Cache::has('component_access_token')) {   //若在有效期内直接返回
        ...
        if (time() < $this->platform_component_access_token_expire) {
        ...
        if ($result) {
            ...
            Log::info('check_platform_access_token $json:' . print_r($json, true));
            //$this->log($json);
            if (!$json || isset($json['errcode'])) {
                Log::info('check_platform_access_token $data:'.print_r($data, true));
            ...
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 0;
            $expire = $expire + time();

            \Illuminate\Support\Facades\DB::table("weixin_conf")->where(['name' => 'platform_component_access_token'])->update(['value' => $this->platform_component_access_token]);
            \Illuminate\Support\Facades\DB::table("weixin_conf")->where(['name' => 'platform_component_access_token_expire'])->update(['value' => $expire]);
            //将access_tonken缓存1小时50分钟
            Cache::put('platform_access_token', $json['component_access_token'], 110);
            return $this->platform_component_access_token;
        }
        return false;
    }
    ...
    {
        //若在有效期 则直接调用
//		if(time()<$this->platform_pre_auth_code_expire){
//			 return $this->platform_pre_auth_code;
//		}
        $data = array("component_appid" => $this->platform_appid);
        $platform_component_access_token = $this->check_platform_access_token();
        $result = $this->http_post(self::API_URL_PREFIX . self::PLATFORM_GET_PRE_AUTH_CODE . $platform_component_access_token, json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
            ...
            \Illuminate\Support\Facades\DB::table("weixin_conf")->where(['name' => 'platform_pre_auth_code'])->update(["value" => $this->platform_pre_auth_code]);
            \Illuminate\Support\Facades\DB::table("weixin_conf")->where(['name' => 'platform_pre_auth_code_expire'])->update(["value" => $expire]);
    ...
    public function platform_api_query_auth($authorization_code, $type = '', $user_id = '')
    {
        ...
        if ($result) {
            ...
            $author_info['expires_in'] = time() + $author_info['expires_in'];
            $author_info['func_info'] = serialize($author_info['func_info']);
            $author_info['business_info'] = serialize($author_info['business_info']);
            $author_info['type'] = intval($type);
            $author_info['is_authorized'] = 1;
            $author_info['user_id'] = $user_id;
            if ($type == 0) {//如果是前台会员公众号且此公众号已存在则不可重复授权
                if (WeixinAccount::where('authorizer_appid', $author_info['authorizer_appid'])->where("user_id", "!=", $user_id)->count()) {
                    $res = [
                        'errcode' => -1,
                        'errmsg' => '该公众号已在授权列表中，如需要请解绑删除后重试',
                    ];
                    return $res;
                }
            }
            if (WeixinAccount::where('authorizer_appid', $author_info['authorizer_appid'])->count()) {
                WeixinAccount::where('authorizer_appid', $author_info['authorizer_appid'])->update($author_info);
            } else {
                $weixin_account = new WeixinAccount();
                $weixin_account->authorizer_appid = $author_info['authorizer_appid'];
                $weixin_account->authorizer_access_token = $author_info['authorizer_access_token'];

                $weixin_account->expires_in = $author_info['expires_in'];
                $weixin_account->authorizer_refresh_token = $author_info['authorizer_refresh_token'];
                $weixin_account->func_info = $author_info['func_info'];
                $weixin_account->business_info = $author_info['business_info'];
                $weixin_account->type = $author_info['type'];
                $weixin_account->is_authorized = $author_info['is_authorized'];
                $weixin_account->user_id = $author_info['user_id'];
                $weixin_account->save();
            }
    ...
    {
        //若在有效期 则直接调用
        Log::info('check_platform_authorizer_token');
        if (time() < $this->authorizer_access_token_expire) {
        ...
        Log::info('check_platform_authorizer_token $data:' . print_r($data, true));
        ...
        if ($result) {
            $json = json_decode($result, true);
            Log::info('check_platform_authorizer_token $json:' . print_r($json, true));
            if (!$json || isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                //dd($json);
            ...
            WeixinAccount::where(["authorizer_appid" => $this->authorizer_appid])->update($data);
            return $this->authorizer_access_token;
        }
        return false;
    ...
    public function platform_get_authrizer_info()
    {
        ...
        if ($result) {
            ...
            if(isset($auth_info['MiniProgramInfo']))
            {
                $miniprograminfo = serialize($auth_info['MiniProgramInfo']);
                unset($auth_info['MiniProgramInfo']);
                $auth_info['miniprograminfo'] = $miniprograminfo;
                $auth_info['xcx'] = 1;

                //只保留一个小程序
                $auth_use_id = WeixinAccount::where(['authorizer_appid' => $auth_info['authorizer_appid']])->first()->user_id;
                WeixinAccount::where(['user_id' => $auth_use_id,"xcx"=>1])->where('authorizer_appid', '<>', $auth_info['authorizer_appid'])->delete();

            }

            WeixinAccount::where(['authorizer_appid' => $auth_info['authorizer_appid']])->update($auth_info);

            if($auth_info['xcx']==1)
            {
                //设置业务域名
                $res = \App\Helper::invoke("app.service/miniprogram/setdomain",["user_id"=>$auth_use_id]);
                if($res['errcode'])
                {
                    dd($res);
                }
        ...
        if ($result) {
            ...
            if (!$json || $json['errcode'] != 0) {
                ...
                if ($json['errmsg'] == 'ok') {
                    ...
                    $auth_info['authorizer_appid'] = $this->authorizer_appid;
    ...
    public function addRecord()
    {
        ...
        $data['user_id'] = 0;
        if ($data['openid']) {
            $re = \Illuminate\Support\Facades\DB::table("weixin_api_get_record")->where(['openid' => $data['openid']])->value("openid");
            //$this->log($data);
            if ($re) {
                \Illuminate\Support\Facades\DB::table("weixin_api_get_record")->where(['openid' => $data['openid']])->update($data);
            } else {
                \Illuminate\Support\Facades\DB::table("weixin_api_get_record")->insert($data);
            }
        }
    }
    ...
    public function setRevData($data = array())
    {
        $this->_receive = $data;
    }
    ...
    public function transmitService()
    {
        ...
        $msg = array(
            ...
            'MsgType' => self::MSGTYPE_transfer_customer,
            'CreateTime' => time(),
            //'FuncFlag'=>$FuncFlag
        );
        $this->Message($msg);
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    {
        Log::info('uploadForeverMedia checkAuth ');
        if (!$this->checkAuth()) {
            Log::info('uploadForeverMedia checkAuth fail!!!');
            return false;
        }
        ...
        Log::info('uploadForeverMedia23333');
        if ($result) {
            Log::info('uploadForeverMedia $result:' . print_r($result, true));
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
    ...
    {
        Log::info('getForeverList');
        if (!$this->checkAuth()) return false;
        Log::info('getForeverList auth');
        $data = array(
            'type' => $type,
            'offset' => $offset,
            'count' => $count,
        ...
        if ($result) {
            Log::info('getForeverList $result:' . print_r($result, true));
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
    ...
    public function sendMassMessage($data)
    {
        ...
        if ($result) {
            $json = json_decode($result, true);
            Log::debug('111 sendMassMessage $json:'.print_r($json, true));
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getQRCode($scene_id, $type = 0, $expire = 1800)
    {
        ...
        Log::info('getQRCode $data'.print_r($data, true));
        $result = $this->http_post(self::API_URL_PREFIX . self::QRCODE_CREATE_URL . 'access_token=' . $this->access_token, self::json_encode($data));

        if ($result) {
            $json = json_decode($result, true);
            Log::info('getQRCode $json'.print_r($json, true));
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
    ...
    public function getDatacube($type, $subtype, $begin_date, $end_date = '')
    {
        ...
        Log::info('getDatacube $data:'.print_r($data, true));
        ...
        if ($result) {
            $json = json_decode($result, true);
            Log::info('getDatacube $json:'.print_r($json, true));
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return false;
...
//	        }
    ...
    public function addTemplateMessage($tpl_id)
    {
        ...
        if ($result) {
            $json = json_decode($result, true);
            $this->log($json);
            \Illuminate\Support\Facades\Log::warn("add_template_fail:".print_r($json,true));
            //var_dump($json);
//	        print_r($data);
//	   		die();
            if (!$json || !empty($json['errcode'])) {
    ...
    {
        if (!$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::TEMPLATE_GET_ALL_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            $this->log($json);
            //var_dump($json);
...
//	   		die();
    ...
    public function get_access_token()
    {
        ...
        $access_token = \Illuminate\Support\Facades\Redis::get($key);
        if ($access_token == null) {
            $data = array(
                "component_appid" => $this->platform_appid,
                "component_appsecret" => $this->platform_appsecret,
                "component_verify_ticket" => $this->platform_component_verify_ticket
            );

            $token_info = $this->http_post("https://api.weixin.qq.com/cgi-bin/component/api_component_token", json_encode($data));
            $token_info = json_decode($token_info, true);
            \Illuminate\Support\Facades\Redis::set($key, $token_info['component_access_token']);
            \Illuminate\Support\Facades\Redis::expire($key, $token_info['expires_in']);
            return $token_info['component_access_token'];

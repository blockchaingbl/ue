...
var widgetsSlider = $.widget( "ui.slider", $.ui.mouse, {
	...
	_mouseCapture: function( event ) {
		...
		index = 1; //by hc强制在range选择只禁用左侧移动点
...
$.widget( "ui.draggable", $.ui.mouse, {
	...
	mouseFirstMove:false, //by hc增加mouse首次移动的标识
	mouseOffsetX:0, //by hc 有角度时mouse的x偏移量，在首次移动时获取
	mouseOffsetY:0, //by hc 有角度时mouse的y偏移量，在首次移动时获取
	...
	_mouseDrag: function( event, noPropagation ) {
		...
		// by hc 增加关于偏移量的修正
		var oPosition = $(this.helper[ 0 ]).position(); //by hc获取原始地址	
		this.helper[ 0 ].style.left = this.position.left - this.mouseOffsetX + "px";
		this.helper[ 0 ].style.top = this.position.top - this.mouseOffsetY + "px";
		var nPosition = $(this.helper[ 0 ]).position(); //by hc获取原始地址

		// by cjy 参考线扩展
		if($(this.helper[ 0 ]).hasClass('f_component')){
			FEditorLine.updatePosition($(this.helper[ 0 ]));
			FEditorLine.drawLine($(this.helper[ 0 ]));
		}

		
		if(this.mouseFirstMove)
		{
			this.mouseOffsetX = nPosition.left - oPosition.left;
			this.mouseOffsetY = nPosition.top - oPosition.top;
			
			this.helper[ 0 ].style.left = this.position.left - this.mouseOffsetX + "px";
			this.helper[ 0 ].style.top = this.position.top - this.mouseOffsetY + "px";

			this.mouseFirstMove = false;	
		}
...
$.widget( "ui.resizable", $.ui.mouse, {
    ...
    rotating:false, //by hc 当前操作是否为旋转
    degree:0, //by hc
	rc:{}, //旋转的圆心坐标
    options: {
        alsoResize: false,
    ...
    _destroy: function() {
        ...
        var wrapper,
            _destroy = function( exp ) {
                ...
                $(exp).find(".ui-resizable-line").remove();
            };
    ...
    _setupHandles: function() {
        ...
        if ( this.handles.constructor === String ) {
            ...
            if ( this.handles === "all" ) {
                this.handles = "n,e,s,w,se,sw,ne,nw,r"; // add r by hc
            }
            ...
            for ( i = 0; i < n.length; i++ ) {
                ...
                if(handle=="r")
                {
                    //添加一个旋转的UI hc
                    axis = $( "<div>" );
                    hname = "ui-resizable-r";
                    this._addClass( axis, "ui-resizable-handle " + hname);
                    axis.css( { zIndex: o.zIndex } );

                    this.handles[ handle ] = ".ui-resizable-r";
                    this.element.append( axis );

                    axis = $( "<div>" );
                    hname = "ui-resizable-rline";
                    this._addClass( axis, "ui-resizable-handle " + hname);
                    axis.css( { zIndex: o.zIndex-10 } );
                    this.element.append( axis );
                }
                else
                {
                    ...
                    //画线
                    var lineClass = "ui-resizable-line-" + handle;
                    if($(this.element).find("."+lineClass).length==0)
                    {
                    axis = $( "<div>" );
                    this._addClass( axis, "ui-resizable-line " + lineClass);

                    axis.css( { zIndex: o.zIndex-1 } );
                    //this.handles[ handle ] = "." + lineClass;
                    this.element.append( axis );
                    }
        ...
        this._renderAxis = function( target ) {
            ...
            for ( i in this.handles ) {
                ...
                if ( this.elementIsWrapper &&
                    ...
                    padWrapper = /sw|ne|nw|se|n|s|r/.test( i ) ?
                        axis.outerHeight() :
                        axis.outerWidth();
        ...
        this._handles.on( "mouseover", function() {
            if ( !that.resizing ) {
                if ( this.className ) {
                    axis = this.className.match( /ui-resizable-(se|sw|ne|nw|n|e|s|w|r)/i );
                }
    ...
    _mouseCapture: function( event ) {
        ...
        //by hc handle是ui-rotate时，表示为旋转
        if($(event.target).hasClass("ui-resizable-r"))
        {
            this.rotating = true;
            var component = this.ui().element[0];
            this.degree = $(component).css("rotate");
			 
			 
			 //取旋转圆心
			 
			var nw = $(component).find(".ui-resizable-nw").offset(); //左上			
			var se = $(component).find(".ui-resizable-se").offset(); //右下
			
			
			this.rc.x = nw.left + (se.left - nw.left) * 0.5;
			this.rc.y = nw.top + (se.top - nw.top) * 0.5;
    ...
    _mouseDrag: function( event ) {
        ...
        if(this.rotating)
        {
			...
			//$("#point").css("left",this.rc.x);
			//$("#point").css("top",this.rc.y);
			
			var moveX = ( event.pageX - this.rc.x ) || 0,
				moveY = ( this.rc.y - event.pageY ) || 0;
			
			var degree = 0;
			if(moveX>=0&&moveY>=0)
			{
				//第一象限
				degree = 180/Math.PI*Math.atan(moveX/moveY);
			}
			else if(moveX>=0&&moveY<0)
			{
				//第二象限
				degree = 180 + 180/Math.PI*Math.atan(moveX/moveY);
			}
			else if(moveX<0&&moveY<0)
			{
				//第三象限
			...
			//console.log(parseInt(moveX)+" "+parseInt(moveY)+" "+parseInt(degree));
            var component = this.ui().element[0];
            
            $(component).css("rotate",degree);

            this.degree = degree;

            if(this.options.rotate!=null)
            this.options.rotate.call(null);

            // by cjy 参考线扩展
            FEditorLine.updatePosition($(this.ui().element[0]));
...
(function($) {
    ...
    $.cssHooks['rotate'] = {
        get: function(elem, computed, extra){
            ...
            if (property) {
                return eval('get'+$(elem).css('transform'));
